...
Se non trovi nessun codice modello: {"sku":null,"confidence":0}`;
      ...
      try {
        ...
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error('timeout-gemini')), 9000)
        );
        const resp = await Promise.race([fetchPromise, timeoutPromise]);

        if (!resp.ok) {
          const errText = await resp.text();
          return cors(JSON.stringify({ sku: `ERR${resp.status}`, confidence: 0, debug: errText.slice(0, 200) }), 200);
        }

        const data = await resp.json();
        const text = data.candidates?.[0]?.content?.parts?.[0]?.text || '';
        const match = text.match(/\{[^}]+\}/);
        if (!match) return cors(JSON.stringify({ sku: `NO-MATCH:${text.slice(0,30)}`, confidence: 0 }), 200);
        const result = JSON.parse(match[0]);
        return cors(JSON.stringify(result), 200);
      } catch (e) {
        return cors(JSON.stringify({ sku: `CATCH:${e.message.slice(0,40)}`, confidence: 0 }), 200);
      }
    }
