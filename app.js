...
async function runOCR(isManual = false) {
  ...
  // DEBUG — mostra cosa ha letto Gemini (rimuovere dopo i test)
  showToast(`OCR: "${result.sku}" (confidenza: ${result.confidence}%)`);
...
async function captureAndAnalyze(imageSource) {
  ...
  if (!imgEl) {
    const video = document.getElementById('video');
    if (!video) { setScanStatus('DEBUG: video element non trovato'); return null; }
    setScanStatus(`DEBUG: video readyState=${video.readyState}`);
  ...
  setScanStatus(`DEBUG: dimensioni=${srcW}x${srcH}`);
  ...
  setScanStatus(`DEBUG: immagine=${Math.round(base64.length/1024)}KB — invio a Gemini...`);
  ...
  try {
    ...
    setScanStatus(`DEBUG: risposta Worker HTTP ${resp.status}`);
    if (!resp.ok) return null;
    const data = await resp.json();
    setScanStatus(`DEBUG: Gemini risponde → sku=${data.sku} conf=${data.confidence}`);
    return data;
  } catch (e) {
    setScanStatus(`DEBUG: errore fetch → ${e.message}`);
    return null;
  }
}
