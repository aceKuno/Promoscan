...
async function runOCR(isManual = false) {
  ...
  setScanStatus('Lettura codice in corso...');
...
function extractSKU(text) {
  ...
  const candidates = tokens.filter(t => {
    ...
    return base.length >= 9 && base.length <= 16 && /^[A-Z0-9]+$/.test(base);
  });
...
async function captureAndAnalyze(imageSource) {
  ...
  enhanceForOCR(canvas, ctx);

  try {
    const { data } = await Tesseract.recognize(canvas, 'eng');
    const sku = extractSKU(data.text);
    if (!sku) return null;
    return { sku, confidence: 92 };
  } catch (e) {
    return null;
  }
}
