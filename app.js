...
async function runOCR(isManual = false) {
  ...
  if (!result || !result.sku) {
    ...
    else setScanStatus('Ricerca in corso... tieni il telefono fermo sul cartellino.');
    return;
  }
...
═══════════════════════════════════════════════════════════ */
function bindChat() {
  document.getElementById('btn-send').addEventListener('click', () => sendChat('result'));
  document.getElementById('chat-input').addEventListener('keydown', e => { if (e.key === 'Enter') sendChat('result'); });
  document.getElementById('btn-mic').addEventListener('click', e => startRecording('chat-input', e.currentTarget));

  document.getElementById('btn-send-nr').addEventListener('click', () => sendChat('no-result'));
  document.getElementById('chat-input-nr').addEventListener('keydown', e => { if (e.key === 'Enter') sendChat('no-result'); });
