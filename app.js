...
/* Parsing data: gestisce DD/MM/YYYY, DD-MM-YYYY, YYYY-MM-DD */
function parsePromoDate(s) {
  if (!s) return null;
  const t = s.trim();
  // YYYY-MM-DD
  ...
  // Fallback: lascia provare al parser nativo
  const d = new Date(t);
  return isNaN(d.getTime()) ? null : d;
}

/* Status dinamico calcolato sul momento dello scan, indipendente dalla colonna Status del CSV */
function computeDynamicStatus(promo, refDate = new Date()) {
  const today = new Date(refDate.getFullYear(), refDate.getMonth(), refDate.getDate());
  const start  = parsePromoDate(promo.StartDate);
  const end    = parsePromoDate(promo.EndDate);
  const regEnd = parsePromoDate(promo.RegistrationEnd) || end;

  if (start && today < start) return 'In partenza';
  if (regEnd && today > regEnd) return 'Chiusa';
  if (end && today > end && regEnd && today <= regEnd) return 'Solo registrazione';
  if (start && end && today >= start && today <= end) return 'Live';
  return 'Sconosciuto';
}
...
function searchPromos(sku) {
  ...
  return state.promoData.filter(row => {
    const status = computeDynamicStatus(row);
    if (status !== 'Live' && status !== 'Solo registrazione') return false;
...
function pickBestSkuFromCandidates(candidates) {
  ...
  const cleaned = candidates
    .map(c => (c || '').toUpperCase().trim().replace(/\s+/g, ''))
    .filter(c => c.length >= 6 && /^[A-Z0-9/-]+$/.test(c));

  if (!cleaned.length) return null;

  // Pass 1: exact match against DB
  for (const c of cleaned) {
    const base = c.split('/')[0];
    if (knownBaseToFull.has(base)) return { sku: knownBaseToFull.get(base), confidence: 98 };
  }

  // Pass 2: fuzzy match against DB (≤1 edit, conservative)
  let best = null;
  for (const c of cleaned) {
    const cBase = c.split('/')[0];
    for (const [base, full] of knownBaseToFull) {
      if (Math.abs(base.length - cBase.length) > 1) continue;
      const dist = levenshtein(cBase, base);
      if (!best || dist < best.dist) best = { dist, full };
    }
  }
  if (best && best.dist <= 1) return { sku: best.full, confidence: 92 };

  // Pass 3: nessun match in DB → mostra comunque il primo candidato (Gemini lo ha scelto come più prominente)
  // L'utente vedrà la schermata di conferma, poi "nessuna promo per questo prodotto"
  return { sku: cleaned[0], confidence: 90 };
}
...
async function captureAndAnalyze(imageSource) {
  ...
  try {
    ...
    const picked = pickBestSkuFromCandidates(data.candidates || []);
    return picked ? { sku: picked.sku, confidence: picked.confidence } : null;
  } catch (e) {
    return null;
  }
}
...
function buildPromoCard(promo) {
  ...
  const dynStatus = computeDynamicStatus(promo);

  card.innerHTML = `
    <div class="promo-card-header">
      <div class="promo-badges">
        ${getStatusBadge(dynStatus)}
        ${promo.Cumulabile === 'Si'
...
async function sendChat(screen, isVoice = false) {
  ...
  const promosSummary = state.currentPromos.map(p => ({
    id: p.PromoID, nome: p.PromoName, stato: computeDynamicStatus(p),
