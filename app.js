...
function searchPromos(sku) {
  ...
  return state.promoData.filter(row => {
    ...
    // DB ha suffisso "/XX": confronta solo la parte prima dello slash
...
async function initOCR() {
  try {
    ...
    await state.tesseractWorker.setParameters({
      ...
      tessedit_pageseg_mode: '7', // single text line — migliore per codici prodotto
    });
...
/* ═══════════════════════════════════════════════════════════
   SPEECH — RICONOSCIMENTO VOCALE E SINTESI
═══════════════════════════════════════════════════════════ */
...
function startRecording(inputId, btnEl) {
  if (!recognition) {
    showToast('Riconoscimento vocale non supportato su questo browser.');
    return;
  }
  if (isRecording) {
    recognition.stop();
  ...
  recognition.onerror = () => {
    isRecording = false;
    btnEl.classList.remove('recording');
    showToast('Errore microfono. Controlla i permessi del browser.');
  };
...
function bindChat() {
  ...
  micBtn.addEventListener('click', () => startRecording('chat-input', micBtn));
  ...
  const micBtnNr = document.getElementById('btn-mic-nr');
  ...
  micBtnNr.addEventListener('click', () => startRecording('chat-input-nr', micBtnNr));
}
...
async function sendChat(screen) {
  ...
  try {
    ...
    speak(answer);
    // Aggiungi pulsante per risentire / fermare
    if (window.speechSynthesis) {
      const speakBtn = document.createElement('button');
      speakBtn.className = 'msg-speak';
      speakBtn.innerHTML = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg> Riascolta`;
