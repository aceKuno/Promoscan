...
function pickBestSkuFromCandidates(candidates) {
  ...
  // Pass 2: fuzzy match against DB (≤2 edits)
  let best = null;
  for (const c of cleaned) {
    const cBase = c.split('/')[0];
    for (const [base, full] of knownBaseToFull) {
      if (Math.abs(base.length - cBase.length) > 2) continue;
      const dist = levenshtein(cBase, base);
      if (!best || dist < best.dist) best = { dist, full, raw: c };
    }
  }
  if (best && best.dist <= 2) return { sku: best.full, confidence: 92, source: `fuzzy(${best.dist})` };

  // Pass 3: no DB match — return the longest candidate raw, low confidence
  if (cleaned.length) {
    const longest = cleaned.sort((a, b) => b.length - a.length)[0];
    return { sku: longest, confidence: 70, source: 'raw' };
  }
  return null;
}
...
async function captureAndAnalyze(imageSource) {
  ...
  try {
    ...
    if (!cands.length) return { sku: null, confidence: 0, _debug: `nessun candidato | raw=${(data._raw||'').slice(0,80)}` };
    const picked = pickBestSkuFromCandidates(cands);
    if (!picked) return { sku: null, confidence: 0, _debug: `cand=[${cands.join(',')}] no match` };
    return { sku: picked.sku, confidence: picked.confidence, _debug: `cand=[${cands.slice(0,3).join(',')}] → ${picked.source}: ${picked.sku}` };
  } catch (e) {
