...
async function captureAndAnalyze(imageSource) {
  ...
  if (!imgEl) {
    const video = document.getElementById('video');
    if (!video || video.readyState < 3) return null; // aspetta HAVE_FUTURE_DATA
    imgEl = video;
  }

  // Dimensioni sorgente — su alcuni mobile readyState ok ma videoWidth ancora 0
  ...
  // Ridimensiona a max 640px per ridurre payload
  ...
  if (!base64 || base64.length < 100) return null; // canvas vuoto
...
function handleDetectedCode(sku, confidence, isManual) {
  ...
  if (confidence >= CONFIG.CONFIDENCE_HIGH) {
    ...
    if (isManual) showScreen('low');
    else {
      setScanStatus(`Codice rilevato con bassa confidenza (${Math.round(confidence)}%). Riprova.`);
      startAutoScan();
    }
  }
}
...
function startRecording(inputId, btnEl) {
  const SR = window.SpeechRecognition || window.webkitSpeechRecognition;
  if (!SR) {
    showToast('Riconoscimento vocale non supportato su questo browser. Usa Chrome.');
    return;
  }
...
═══════════════════════════════════════════════════════════ */
function bindChat() {
  document.getElementById('btn-send').addEventListener('click', () => { lastInputWasVoice = false; sendChat('result'); });
  document.getElementById('chat-input').addEventListener('keydown', e => { if (e.key === 'Enter') { lastInputWasVoice = false; sendChat('result'); } });
  document.getElementById('btn-mic').addEventListener('click', e => startRecording('chat-input', e.currentTarget));

  document.getElementById('btn-send-nr').addEventListener('click', () => { lastInputWasVoice = false; sendChat('no-result'); });
  document.getElementById('chat-input-nr').addEventListener('keydown', e => { if (e.key === 'Enter') { lastInputWasVoice = false; sendChat('no-result'); } });
  document.getElementById('btn-mic-nr').addEventListener('click', e => startRecording('chat-input-nr', e.currentTarget));
}
...
async function sendChat(screen) {
  ...
  try {
    ...
    if (lastInputWasVoice) speak(answer);
