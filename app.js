...
function looksLikeValidProductCode(s) {
  if (!s) return false;
  const base = s.split('/')[0];
  if (base.length < 9 || base.length > 16) return false;
  if (!/^[A-Z0-9]+$/.test(base)) return false;
  // Must contain both letters and digits
  if (!/[A-Z]/.test(base) || !/[0-9]/.test(base)) return false;
  return true;
}
...
function pickBestSkuFromCandidates(candidates) {
  ...
  const cleaned = candidates
    ...
    .filter(c => c.length >= 7 && /^[A-Z0-9/-]+$/.test(c));

  // Pass 1: exact match against DB
  for (const c of cleaned) {
    const base = c.split('/')[0];
    if (knownBaseToFull.has(base)) return { sku: knownBaseToFull.get(base), confidence: 98, source: 'exact' };
  }
  ...
  if (best && best.dist <= 1) return { sku: best.full, confidence: 92, source: `fuzzy(${best.dist})` };

  // Pass 3: not in DB but looks like a valid product code — pass through with medium confidence
  for (const c of cleaned) {
    if (looksLikeValidProductCode(c)) return { sku: c, confidence: 88, source: 'valid-pattern' };
  }

  // Pass 4: weird candidate — low confidence (will trigger "low" screen)
  const longest = cleaned.sort((a, b) => b.length - a.length)[0];
  if (longest) return { sku: longest, confidence: 60, source: 'raw' };
  return null;
}
